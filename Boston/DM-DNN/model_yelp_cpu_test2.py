...
class NN(nn.Module):
    ...
    def forward(self, words_embed, u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_u_dict, v_v_dict):
        ...
        # GAT
        # x_u = self.gat1_user(self.user_embed2, torch.tensor(ug_u_u2).long())
        # x_u = F.relu(x_u)
        # # x_u = F.leaky_relu(x_u)
        # x_u = F.dropout(x_u, p=0.3)
        # x_u = self.gat2_user(x_u, torch.tensor(ug_u_u2).long())
        #
        # x_v = self.gat1_business(self.event_embed2, torch.tensor(ug_v_v2).long())
        # x_v = F.relu(x_v)
        # # x_v = F.leaky_relu(x_v)
        # x_v = F.dropout(x_v, p=0.3)
        ...
        score = torch.matmul(user_embed, event_embed.T)
