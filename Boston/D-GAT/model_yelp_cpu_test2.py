...
class NN(nn.Module):
    ...
    def compute_embedding(self, edge_user, edge_event, v_v_dict):
        # 映射各个矩阵的embed
        # 用户事件交互
        ...
        # user_event_embed = torch.cat((u_v_vid_embed), dim=0)
        ...
        # event_user_embed = torch.cat((v_u_uid_embed), dim=0)
        # user_event_embed = torch.cat((u_v_vid_embed, u_v_lati_longi_embed, u_v_item_starts_embed, u_v_review_count_embed), dim=0)
        # event_user_embed = torch.cat((v_u_date_embed, v_u_user_review_count, v_u_words_embed), dim=0)

        return u_v_vid_embed, v_u_uid_embed

    def forward(self, u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_u_dict, v_v_dict):
        ...
        user_event_embed, event_user_embed = self.compute_embedding(u_v_matrix, v_u_matrix, v_v_dict)
