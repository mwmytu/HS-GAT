...
class LinearEmbed(nn.Module):
    def __init__(self,):
        ...
        self.out = nn.Linear(64, config.n_emb)
        self.training = True
...
class NN(nn.Module):
    def __init__(self, edge_user, edge_event, in_channels, out_channels, user_len, business_len, num_embed, w_out):
        ...
        # 连续量使用线性层获得embed
        self.u_v_lati_longi_embed = LinearEmbed()
    ...
    def compute_embedding(self, words_embed, edge_user, edge_event, v_v_dict):
        ...
        u_v_lati_longi_embed = torch.reshape(self.u_v_lati_longi_embed(torch.reshape(edge_user[1].float(), (-1, 1))), (1, edge_user.shape[1], edge_user.shape[2], self.num_emed))
    ...
    def forward(self, words_embed, u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_u_dict, v_v_dict):
        ...
        for i in range(user_event_embed.shape[0]):
            ...
            user_embed[i], beta_user = self.oneGraphAttention_user(user_event_embed[i])
        ...
        for i in range(event_user_embed.shape[0]):
            ...
            event_embed[i], beta_event = self.oneGraphAttention_event(event_user_embed[i])
        ...
        # score = torch.reshape(score, (x_u.shape[0], x_v.shape[0]))
