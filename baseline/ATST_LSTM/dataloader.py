...
def train_deal(dataset, train_data):
    ...
    for cur_user in dataset["user_id"].unique():
        user_set = set()
        # user_list = []
        for cur_business in dataset[dataset["user_id"] == cur_user]["business_id"]:
            user_set.add(cur_business)
            # user_list.append(cur_business)
    ...
    #   3.项目评论次数矩阵
    u_v_review_count = np.zeros((user_len, config.neighbor_num))
    #   4.项目id矩阵
    ...
    for k, v in enumerate(u_u_dict):
        ...
        for j in range(len(u_u_dict[v])):
            ...
            u_v_item_stars[v][j] = \
                train_data[(train_data.user_id == v) & (train_data.business_id == u_u_dict[v][j])][
                    "timestamp_index"].unique()[0]
            u_v_review_count[v][j] = \
                train_data[(train_data.user_id == v) & (train_data.business_id == u_u_dict[v][j])][
                    "item_review_count"].unique()[0]
            i += 1

    u_v_matrix = np.stack((u_v_vid, u_v_lati_longi, u_v_item_stars, u_v_review_count), axis=0)
    ...
    #   2.加入时间矩阵
    v_u_yelping_since = np.zeros((business_len, config.neighbor_num))
    #   3.用户评论次数矩阵
    ...
    for k, v in enumerate(v_v_dict):
        for j in range(len(v_v_dict[v])):
            ...
            v_u_date[v][j] = \
                train_data[(train_data.business_id == v) & (train_data.user_id == v_v_dict[v][j])]["timestamp_index"].unique()[
                    0]
            # v_u_yelping_since[cur_bus][uid] = v_u_fea[(v_u_fea.business_id == cur_bus) & (v_u_fea.user_id == uid)]["yelping_since"].unique()[0]
            v_u_review_count[v][j] = \
                train_data[(train_data.business_id == v) & (train_data.user_id == v_v_dict[v][j])][
                    "review_count"].unique()[0]
            words_list.append(
                train_data[(train_data.business_id == v) & (train_data.user_id == v_v_dict[v][j])]["text"].unique()[0])

    v_u_matrix = np.stack((v_u_uid, v_u_date, v_u_review_count), axis=0)
    ...
    return u_v_matrix, v_u_matrix, target, words_list, u_u_dict, v_v_dict, u_u_dict_all
...
def get_matrix_factor():
    f_bus = open("../data/yelp_academic_dataset_business.json", encoding="utf8")
...
def getData():
    ...
    for i in range(dataset.shape[0]):
        timestamp_list.append(get_timestamp(dataset['date'].loc[i]))

    dataset["timestamp"] = pd.DataFrame(timestamp_list)
    dataset = dataset.sort_values(by='timestamp')
    index_list_ = []
    front_time, font_index = dataset['timestamp'].loc[0], 0
    for i in range(dataset.shape[0]):
        if dataset['timestamp'].loc[i] == str(front_time): index_list_.append(font_index)
        else:
            index_list_.append(font_index+1)
            font_index += 1
            front_time = dataset['timestamp'].loc[i]
    dataset["timestamp_index"] = pd.DataFrame(index_list_)
    # 对数据集的字段进行处理
    # 人类按照心情变化，一天可以分为4个阶段
