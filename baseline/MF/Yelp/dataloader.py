...
def train_deal(dataset, train_data):
    ...
    for k, v in enumerate(v_v_dict):
        for j in range(len(v_v_dict[v])):
            ...
            v_u_date[v][j] = \
                dataset[(dataset.business_id == v) & (dataset.user_id == v_v_dict[v][j])]["date"].unique()[
                    0]
            ...
            v_u_review_count[v][j] = \
                dataset[(dataset.business_id == v) & (dataset.user_id == v_v_dict[v][j])]["review_count"].unique()[0]
            words_list.append(
                dataset[(dataset.business_id == v) & (dataset.user_id == v_v_dict[v][j])]["text"].unique()[0])
    ...
    return u_v_matrix, v_u_matrix, uv_tag, words_list, u_u_dict, v_v_dict
...
if __name__ == '__main__':
    ...
    dataset, user_item_matrix = get_matrix_factor(dataset, train_data)
