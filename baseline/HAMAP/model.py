...
class HAMAP(nn.Module):
    def __init__(
        ...
        self.dense1_ = nn.Linear(config.n_emb, 560)
    ...
    def forward(self, edge_user, edge_poi, u_v_edge, v_u_edge):
        ...
        # user_embed_final = torch.squeeze(self.dense2_(user_embed_final.permute((0, 2, 1))))
        ...
        user_check_ins_embed2 = torch.zeros((check_ins_embed.shape[0], 5, 560))
        for i in range(check_ins_embed.shape[0]):
        ...
        score_final = user_score2 + user_score1
        return score_final
