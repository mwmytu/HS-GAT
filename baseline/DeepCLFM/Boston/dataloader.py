...
def train_deal(dataset, train_data):
    ...
    for cur_business in dataset["business_id"].unique():
        item_user_word_ = []
        item_user = dataset[dataset.business_id == cur_business]["user_id"]
        for cur_user in item_user:
            item_word = dataset[(dataset.business_id == cur_business) & (dataset.user_id == cur_user)]["text"].unique()[
                0]
    ...
    # 将每个用户的评论组合成语料库，共享语义空间
    if not os.path.exists("./word_data/user_words_str_400001.csv"):
        user_words_str_list1d = []
        for i in range(len(user_item_word_list)):
            user_words_list1d = []
            for j in range(len(user_item_word_list[0])):
                user_words_list1d += [str(k) for k in user_item_word_list[i][j]]
            user_words_str = ",".join(user_words_list1d)
            user_words_str_list1d.append(user_words_str)
        user_words_str_df = pd.DataFrame(user_words_str_list1d, columns=["words"])
        user_words_str_df.to_csv("./word_data/user_words_str_400001.csv", encoding="utf8")
    # 将每个项目的评论组合成语料库，共享语义空间
    if not os.path.exists("./word_data/item_words_str_400001.csv"):
        item_words_str_list1d = []
        for i in range(len(item_user_word_list)):
            item_words_list1d = []
            for j in range(len(item_user_word_list[0])):
                item_words_list1d += [str(k) for k in item_user_word_list[i][j]]
            item_words_str = ",".join(item_words_list1d)
            item_words_str_list1d.append(item_words_str)
        item_words_str_df = pd.DataFrame(item_words_str_list1d, columns=["words"])
        item_words_str_df.to_csv("./word_data/item_words_str_400001.csv", encoding="utf8")
    ...
    return target, u_u_dict, train_data
...
def get_data():
    ...
    dataset["user_id"].replace(user_ids_invmap, inplace=True)
    # 将经纬度转化为离散量
    longitude_uni = dataset["longitude"].unique()
    latitude_uni = dataset["latitude"].unique()
    longitude_ids_invmap = {id_: i for i, id_ in enumerate(longitude_uni)}
    latitude_ids_invmap = {id_: i for i, id_ in enumerate(latitude_uni)}
    dataset["longitude"].replace(longitude_ids_invmap, inplace=True)
    dataset["latitude"].replace(latitude_ids_invmap, inplace=True)
    dataset.reset_index(drop=True, inplace=True)
...
if __name__ == '__main__':
    ...
    uv_tag, u_u_dict, train_data = train_deal(dataset, train_data)
