...
from utils import delete_stopwords, \
    cut_words_key, f1_score_topn, \
    get_glove, charToEmbed, precision_topn, hits, recall_topn
...
model.load_state_dict(torch.load("./model/2023-01-12_13_47_34_model.pth"))
# model.load_state_dict(torch.load("./model/2022-12-02_19_36_42_model.pth"))
...
with torch.no_grad():
    for q in range(100):
        ...
        aver_recall_list, aver_pre_list, aver_f1_list = [], [], []
        for i in range(50):
            output = model(u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_i_dict, v_v_dict)
