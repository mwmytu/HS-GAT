...
class NN(nn.Module):
    ...
    def forward(self, words_embed, u_v_matrix, v_u_matrix, ug_v_v2, ug_u_u2, v_v_dict, user, item_i, item_j):
        ...
        # 初始化用户和事件embed完成
        user_embed2 = torch.zeros((user_event_embed.shape[0], self.num_emed))
        event_embed2 = torch.zeros((event_user_embed.shape[0], self.num_emed))
        for i in range(user_event_embed.shape[0]):
            user_embed2[i] = self.oneGraphAttention_user(user_event_embed[i])

        for i in range(event_user_embed.shape[0]):
            event_embed2[i] = self.oneGraphAttention_event(event_user_embed[i])

        # GAT
        x_u = self.gat1_user(user_embed2, torch.tensor(ug_u_u2).long())
        x_u = F.relu(x_u)
        # x_u = F.leaky_relu(x_u)
        x_u = F.dropout(x_u, p=0.3)
        x_u = self.gat2_user(x_u, torch.tensor(ug_u_u2).long())

        x_v = self.gat1_business(event_embed2, torch.tensor(ug_v_v2).long())
        x_v = F.relu(x_v)
