...
class NN(nn.Module):
    def __init__(self, edge_user, edge_event, in_channels, out_channels, user_len, business_len, num_embed, w_out):
        ...
        self.conv_user = GTConv(self.in_channels, self.out_channels, num_embed, self.edge_user.shape[1],
                                5)
        self.conv_event = GTConv(self.in_channels, self.out_channels, num_embed, self.edge_event.shape[1],
                                5)
        ...
        self.gru = nn.GRU(input_size=50, hidden_size=config.n_emb, num_layers=2, batch_first=False)
    ...
    def compute_embedding(self, words_embed, edge_user, edge_event, v_v_dict):
        ...
        u_v_vid_embed = torch.reshape(self.u_v_vid_embed(torch.reshape(edge_user[0].long(), (1, -1))),
                                      (1, edge_user.shape[1], edge_user.shape[2], self.num_emed))
        u_v_lati_longi_embed = torch.reshape(self.u_v_lati_longi_embed(torch.reshape(edge_user[1].long(), (1, -1))),
                                             (1, edge_user.shape[1], edge_user.shape[2], self.num_emed))
        # u_v_item_starts_embed = torch.reshape(self.u_v_item_stars_embed(torch.reshape(edge_user[2].long(), (1, -1))),
        #                                       (1, edge_user.shape[1], edge_user.shape[2], self.num_emed))
        ...
        v_u_uid_embed = torch.reshape(self.v_u_uid_embed(torch.reshape(edge_event[0].long(), (1, -1))),
                                      (1, edge_event.shape[1], edge_event.shape[2], self.num_emed))
        # v_u_date_embed = torch.reshape(self.v_u_date_embed(torch.reshape(edge_event[1].long(), (1, -1))),
        #                                (1, edge_event.shape[1], edge_event.shape[2], self.num_emed))
        ...
        user_event_embed = torch.cat((u_v_vid_embed, u_v_lati_longi_embed), dim=0)
        ...
        event_user_embed = torch.cat((v_u_uid_embed, v_u_words_embed), dim=0)
        # user_event_embed = torch.cat((u_v_vid_embed, u_v_lati_longi_embed, u_v_item_starts_embed, u_v_review_count_embed), dim=0)
    ...
    def forward(self, words_embed, u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_u_dict, v_v_dict):
        ...
        # x_u_ts = TSNE(n_components=2, random_state=33).fit_transform(x_u.numpy())
        # x_v_ts = TSNE(n_components=2, random_state=33).fit_transform(x_v.numpy())
