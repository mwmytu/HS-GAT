...
class NN(nn.Module):
    ...
    def forward(self, words_embed, u_v_matrix, v_u_matrix, ug_u_u2, ug_v_v2, u_u_dict, v_v_dict):
        ...
        #  no at
        user_event_embed = torch.sum(user_event_embed, dim=0)
        event_user_embed = torch.sum(event_user_embed, dim=0)
        ...
        user_embed = torch.sum(user_event_embed, dim=1)
        event_embed = torch.sum(event_user_embed, dim=1)
